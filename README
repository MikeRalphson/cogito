...
of GIT and start to use it and hack upon it. It can give you some guide,
but the documentation should be certainly consolidated and rewritten.

This mini-HOWTO assumes two facts about the tarball you just got:
	(i)  It is "tracking" some branch
	(ii) It is "tracking" branch called "pasky"

Build it by make, make sure the scripts are in $PATH, and update it to the
latest version by

	git pull pasky

(and repeat that after some time to get the future updates - but see below).
Then you can finally build it, doing

	make

and...  well, that's it. Play around with it a bit. ;-)

This tree is tracked. That is, you can get my latest changes by doing:

	git pull pasky

If you did some local commits in the meantime, git will attempt to merge them
with the pasky branch. You then need to check that the merging went correctly
and without conflicts, possibly do merge-related fixes, and then record the
merge with git commit.

You may want to untrack my tree (to do independent development or re-track to
e.g. linus), like

	git track local

(git track without any parameters will do too). The branch is still there,
though, and you can pull the latest updates again with:

	git pull pasky

You aren't tracking the branch now, though, so the changes won't be applied to
your local tree automatically anymore. However, if there are any changes, two
IDs will be printed (I mean the line saying "Tree change"). Pass those as
parameters to git diff and you will get a diff describing changes from the last
time you pulled. You can also
...
even things like ctime or inode number (!) changed. Actuall diff is printed out
...
will get you the history of my branch. git log with no arguments will default
to the history of the current branch.


For reference, my branch URL is:
